...
const targets = [{ x: 26, y: 65, label: "T-01" }, { x: 69, y: 30, label: "T-02" }, { x: 81, y: 76, label: "T-03" }];
...
export default function Home() {
  ...
  const { coverage, discovered, energy } = getBehaviorMetrics(behavior, tick);
  ...
  return <div className="console-shell">
    ...
    <div className="console-grid">
      <aside className="left-rail" aria-label="Application navigation">
        ...
        <div className="rail-divider" /><div className="control-block"><div className="section-kicker">Active scenario</div><select className="mission-select" aria-label="Select active scenario" defaultValue="search"><option value="search">Search &amp; locate</option><option value="coverage">Area coverage</option><option value="collection">Resource collection</option></select></div>
        <div className="rail-bottom"><div className="section-kicker">Communication</div><strong>LOCAL RELAY</strong></div>
      </aside>

      <main className="mission-main" id="mission">
        <header className="mission-header"><div><div className="eyebrow">Mission / Search &amp; locate</div><h1 className="mission-title">No controller.<br />One coordinated search.</h1><p className="mission-summary">Local sensing, local decisions, and a shared objective across a 10,000-cell disaster zone.</p></div><div className="header-coordinates">OPERATIONAL AREA<br /><b>34.0522° N / 118.2437° W</b><br />GRID RESOLUTION <b>100 × 100</b></div></header>
        <section className="map-card" aria-label="Live swarm simulation map"><div className="map-header"><div className="map-name">SECTOR 14 <span>10,000 CELLS</span></div><div className="map-legend"><span className="legend-chip"><i className="legend-swatch teal" /> Agents</span><span className="legend-chip"><i className="legend-swatch orange" /> Targets</span><span className="legend-chip"><i className="legend-swatch red" /> Offline</span></div></div>
          <svg className="map-svg" viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label={`${activeAgents} active autonomous agents using ${behavior} behavior`}><defs><linearGradient id="terrain" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stopColor="#0a1d24" /><stop offset="1" stopColor="#081217" /></linearGradient><pattern id="grid" width="10" height="10" patternUnits="userSpaceOnUse"><path d="M 10 0 L 0 0 0 10" className="grid-line" fill="none" /></pattern><filter id="softGlow"><feGaussianBlur stdDeviation="1.3" /></filter></defs><rect width="100" height="100" fill="url(#terrain)" /><rect width="100" height="100" fill="url(#grid)" />{[22, 46, 70].map((y) => <path key={y} className="scan-line" d={`M0 ${y} C 28 ${y - 8}, 56 ${y + 8}, 100 ${y - 3}`} fill="none" />)}{[18, 49, 78].map((x) => <path key={x} className="scan-line" d={`M${x} 0 C ${x + 8} 24, ${x - 8} 59, ${x + 3} 100`} fill="none" />)}{visibleAgents.map((agent) => <g key={agent.id} opacity={agent.failed ? .94 : 1}>{!agent.failed && <line className="trail" x1={agent.trailX} y1={agent.trailY} x2={agent.x} y2={agent.y} />}{agent.selected && !agent.failed && <circle className="agent-pulse" cx={agent.x} cy={agent.y} r="4.3" filter="url(#softGlow)" />}<circle className={agent.failed ? "agent-failed" : "agent-dot"} cx={agent.x} cy={agent.y} r={agent.failed ? "1.45" : "1.18"} />{agent.failed && <path d={`M${agent.x - 1.2} ${agent.y - 1.2} L${agent.x + 1.2} ${agent.y + 1.2} M${agent.x + 1.2} ${agent.y - 1.2} L${agent.x - 1.2} ${agent.y + 1.2}`} stroke="#fff2f2" strokeWidth=".4" />}</g>)}{targets.map((target, index) => { const found = index < discovered; return <g key={target.label} opacity={found ? 1 : .35}><circle className={found ? "target-ring" : "target-hidden"} cx={target.x} cy={target.y} r={found ? "4.4" : "2.3"} />{found && <circle className="target-ring" cx={target.x} cy={target.y} r="7" opacity=".33" />}<circle className={found ? "target-core" : "target-hidden"} cx={target.x} cy={target.y} r="1.3" />{found && <text x={target.x + 3} y={target.y - 3} fill="#feb370" fontSize="2.2" fontFamily="DM Mono">{target.label}</text>}</g>})}<text x="3" y="96" fill="#66817a" fontSize="1.8" fontFamily="DM Mono">NORTHING 4.02</text><text x="85" y="8" fill="#66817a" fontSize="1.8" fontFamily="DM Mono">E-14 / 064</text></svg>
          <div className="map-footer"><span>LOCAL SENSORS / 24 m RANGE</span><span>{behaviorOptions.find((option) => option.id === behavior)?.label.toUpperCase()} BEHAVIOR</span></div>
        </section>
      ...
      <aside className="telemetry-panel" aria-label="Simulation telemetry"><div className="telemetry-heading"><h2 className="telemetry-title">Mission telemetry</h2><span className="telemetry-live">LIVE <Activity size={11} style={{ display: "inline", verticalAlign: "-2px" }} /></span></div><section className="telemetry-card"><div className="telemetry-card__title">Objective progress</div><div className="progress-bar"><span style={{ width: `${coverage}%` }} /></div><div className="progress-copy"><span>Area coverage</span><b>{coverage}%</b></div><div className="progress-bar target"><span style={{ width: `${(discovered / 3) * 100}%` }} /></div><div className="progress-copy"><span>Target discovery</span><b>{discovered} / 3</b></div></section><section className="telemetry-card"><div className="telemetry-card__title">Environment</div><div className="parameter-list" style={{ marginTop: 13 }}><div className="parameter-row"><label htmlFor="agents">Swarm size</label><output>{agentCount}</output><input id="agents" className="slider" type="range" min="100" max="1000" step="100" value={agentCount} onChange={(event) => setAgentCount(Number(event.target.value))} /></div><div className="parameter-row"><label htmlFor="failure">Failure profile</label><output>{failureRate}%</output><input id="failure" className="slider" type="range" min="0" max="30" step="5" value={failureRate} onChange={(event) => setFailureRate(Number(event.target.value))} /></div><div className="parameter-row"><label>Communication</label><output>LOCAL</output></div><div className="parameter-row"><label>Sensor radius</label><output>24 m</output></div></div></section><section className="telemetry-card"><div className="telemetry-card__title">Decision log</div><div className="event-list" style={{ marginTop: 13 }}>{events.map((event, index) => <div className="event-row" key={`${event.time}-${index}`}><i className={`event-dot ${event.kind === "warning" ? "warning" : event.kind === "error" ? "error" : ""}`} /><div><time>{event.time}</time>{event.text}</div></div>)}</div></section><button className="secondary-button failure-button" type="button" onClick={triggerFailure}><AlertTriangle size={14} /> Test recovery logic</button><div className="telemetry-card" style={{ marginTop: 12 }}><div className="telemetry-card__title"><Gauge size={12} style={{ display: "inline", verticalAlign: "-2px", marginRight: 6 }} /> System health</div><div className="progress-copy" style={{ marginTop: 10 }}><span>Communication mesh</span><b style={{ color: "#4be4c1" }}>NOMINAL</b></div><div className="progress-copy" style={{ marginTop: 7 }}><span>Objective behavior</span><b>{behaviorOptions.find((option) => option.id === behavior)?.label.toUpperCase()}</b></div></div>
